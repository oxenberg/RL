...
class PolicyNetwork:
    def __init__(self, learning_rate, name='policy_network', retrain=False, mountain_car=False):
        ...
        with variable_scope(self.name):
            ...
            self.W1 = get_variable("W1", [STATE_SIZE, self.neurons], initializer=GlorotNormal(seed=0))
            self.b1 = get_variable("b1", [self.neurons], initializer=tf.zeros_initializer())
            self.W2 = get_variable("W2", [self.neurons, ACTION_SIZE], initializer=GlorotNormal(seed=0))
            self.b2 = get_variable("b2", [ACTION_SIZE], initializer=tf.zeros_initializer())
            if retrain:
                self.W2 = get_variable("W2_retrain", [self.neurons, ACTION_SIZE], initializer=GlorotNormal(seed=0))
...
class Agent:
    ...
    def run(self, discount_factor, learning_rate, learning_rate_value,
        ...
        self.policy = PolicyNetwork(learning_rate, retrain=restore_sess is not None, mountain_car=mountain_car)
        ...
        with Session() as sess:
            ...
            save_path = saver.save(sess, f"/tmp/{self.env_name + transfered}-model.ckpt")
